...
import { AddTabModal, TabCard } from "@/features/tab/components";
...
export const WorkspaceView: React.FC<WorkspaceViewProps> = ({
	...
	const findContainer = useCallback(
		(id: string) => {
			// Handle droppable-{collectionId} format
			if (id.startsWith("droppable-")) {
				const collectionId = id.replace("droppable-", "");
				return collections.some((c) => c.id === collectionId)
					? collectionId
					: undefined;
			}
			// Handle sortable-{collectionId} format
			if (id.startsWith("sortable-")) {
				const collectionId = id.replace("sortable-", "");
				return collections.some((c) => c.id === collectionId)
					? collectionId
					: undefined;
			}
	...
	const collisionDetectionStrategy: CollisionDetection = useCallback(
		(args) => {
			...
			if (
				activeId &&
				(collections.some((c) => c.id === activeId) ||
					(typeof activeId === "string" && activeId.startsWith("sortable-")))
			) {
					...
					droppableContainers: args.droppableContainers.filter(
						(container) =>
							...
							(typeof container.id === "string" &&
								container.id.startsWith("sortable-")),
					),
				});
			}
			...
			if (overId != null) {
				...
				if (overContainer) {
					// Get tabs in the container, excluding the active tab being dragged
					const containerTabs = tabs
						.filter(
							(t) => t.collectionId === overContainer && t.id !== activeId,
						)
						.map((t) => t.id);
	...
	const handleDragOver = (event: DragOverEvent) => {
		...
		const isDraggingCollection =
			collections.some((c) => c.id === active.id) ||
	...
	const handleDragEnd = async (event: DragEndEvent) => {
		...
		if (
			...
			// Extract collection IDs from sortable-{id} format
			const activeCollectionId =
				typeof active.id === "string" && active.id.startsWith("sortable-")
					? active.id.replace("sortable-", "")
					: active.id;
			const overCollectionId =
				typeof over.id === "string" && over.id.startsWith("sortable-")
					? over.id.replace("sortable-", "")
					: over.id;

			const oldIndex = workspaceCollections.findIndex(
				(c) => c.id === activeCollectionId,
			);
			const newIndex = workspaceCollections.findIndex(
				(c) => c.id === overCollectionId,
			);
		...
		// Handle tab move - create history commit
		if (active.data.current?.type === "TAB" && over) {
			const tab = tabs.find((t) => t.id === active.id);
			if (tab) {
				const activeContainer = active.data.current.collectionId as string;
				let overContainer: string;

				if (over.data.current?.type === "TAB") {
					overContainer = over.data.current.collectionId as string;
				} else if (over.data.current?.type === "COLLECTION") {
					overContainer = over.data.current.collectionId as string;
				} else {
					// Handle droppable-{collectionId} format
					const overId = over.id as string;
					overContainer = overId.startsWith("droppable-")
						? overId.replace("droppable-", "")
						: overId;
				}

				// Only create history if the tab actually moved
				if (
					activeContainer !== overContainer ||
					clonedTabs?.some((t) => t.id === tab.id && t.order !== tab.order)
				) {
					...
					// Create history commit for the final move
					await moveTab({
						tabId: active.id as string,
						targetCollectionId: overContainer,
						targetIndex: newIndex,
						shouldPin: overTab?.isPinned,
						skipHistory: false, // Create history on drag end
					});
				}
			}
		}
